...
namespace HLU.Data
{
    class DbVersion
    {
        ...
        public DbVersion(DbBase db, HluDataVersion hluDataset, TableAdapterManager hluTableAdapterMgr)
        {
            ...
            if (_hluDataset.lut_version.IsInitialized && _hluDataset.lut_version.Count == 0)
            {
                if (_hluTableAdapterMgr.lut_versionTableAdapter == null)
                    _hluTableAdapterMgr.lut_versionTableAdapter =
                        new HluTableAdapter<HluDataVersion.lut_versionTable, HluDataVersion.lut_versionRow>(_db);
                _hluTableAdapterMgr.Fill(_hluDataset,
                    new Type[] { typeof(HluDataVersion.lut_versionTable) }, false);
            }
